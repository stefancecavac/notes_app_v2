import { BlockData } from "../../types";
import { TextBlockComponent } from "./BlockComponents/TextBlockComponent";

type blockComponentProps = {
  block: BlockData;
  nextBlock?: BlockData | null;
  singleNoteLoading?: boolean;
};

const BlockComponent = ({ block, nextBlock, singleNoteLoading }: blockComponentProps) => {
  if (singleNoteLoading) return null;

  return (
    <div className={`rounded-lg  transition-all hover:cursor-text  relative group/handle`}>
      {(() => {
        switch (block?.type) {
          case "text":
            return <TextBlockComponent block={block} />;
          case "image":
            return <p>image block : {block.properties.title}</p>;
          case "to-do":
            return <p>todo block : {block.properties.title}</p>;
        }
      })()}
      <div className="absolute   flex-row-reverse gap-1  items-center -left-20 top-1 px-5 hidden group-hover/handle:flex scale-up-center ">
        <div className="btn btn-xs btn-ghost hover:bg-base-300 text-info-content btn-square">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="size-4 "
          >
            <circle cx="12" cy="5" r="1" />
            <circle cx="19" cy="5" r="1" />
            <circle cx="5" cy="5" r="1" />
            <circle cx="12" cy="12" r="1" />
            <circle cx="19" cy="12" r="1" />
            <circle cx="5" cy="12" r="1" />
            <circle cx="12" cy="19" r="1" />
            <circle cx="19" cy="19" r="1" />
            <circle cx="5" cy="19" r="1" />
          </svg>
        </div>

        <button className="btn btn-xs btn-square btn-error btn-ghost text-error hover:text-base-200  transition-all ">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
            />
          </svg>
        </button>
      </div>
    </div>
  );
};

export default BlockComponent;
